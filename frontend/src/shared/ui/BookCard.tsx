...
export function BookCard({ offer, onContact, onFavorite }: BookCardProps) {
  ...
  return (
    ...
    >
      {/* Book image */}
      <div className="w-16 sm:w-20 h-20 sm:h-28 bg-gray-200 rounded flex-shrink-0
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:p-4 lg:gap-0">
        ...
        <div className="flex-1">
          <h3 className="font-bold text-base sm:text-lg mb-1 truncate lg:text-base lg:mb-2">{book.title}</h3>
          <p className="text-gray-600 text-xs sm:text-sm mb-1 truncate lg:text-sm lg:mb-2">by {book.authors.join(", ")}</p>
          <p className="text-xs sm:text-sm text-gray-600 mb-2 lg:text-sm lg:mb-3">Condition: {book.condition}</p>
          
          {/* Price and Exchange */}
          <div className="flex items-center gap-2 sm:gap-4 mb-2 sm:mb-3 lg:flex-col lg:items-start lg:gap-2 lg:mb-4">
            <div className="flex flex-col lg:flex-row lg:items-center lg:gap-2">
              <span className="text-xs text-gray-500 mb-1 lg:mb-0 lg:text-sm">Price:</span>
              <span className="font-bold text-base sm:text-lg lg:text-xl">{price} UAH</span>
            </div>
            {type === 'EXCHANGE' && (
